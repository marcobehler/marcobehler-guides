...
We managed to schedule a single, static, one-off pod. Where is the advantage to `_docker run -d --publish 8080:8080 gcr.io/marco/marcocodes:1.4_`? In this example, there is none.

That's why we unfortunately need to learn two more concepts: `_ReplicaSets_` and `_Deployments_
