...
You can download `_kubectl_`, which is essentially _the_ CLI tool to do everything you ever wanted to do with your Kubernetes cluster https://kubernetes.io/docs/tasks/tools/[here]. That page lists various ways of installing kubectl for your specific operationg system.
...
Just in case you have to, it helps tremendously to use your IDEs Kubernete's support, https://www.jetbrains.com/help/idea/kubernetes.html[IntelliJ IDEA] in my case, to get coding assistance support for Helm charts, Kustomize files et al. Oh, we haven't talked about them yet. Let's do that.
...
If there is any interested in these common `_kubectl_` commands, post a comment down below, and I'll add the most frequently used here, as neatly grouped/sorted list.
...
du mussst wissen welche volumes und shit du bereit˚stellen musst
